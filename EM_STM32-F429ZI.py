...
def clustering_procedure(pca_samples, kind):
    ...
    plt.savefig(save_path + device + "_entries_list_" + date + "_CONFUSION_MATRIX_TIMEDOMAIN_" + str(get_class_name(db)) + "_EM.png", bbox_inches='tight', dpi=100)
    plt.show()
...
def bugs_capture(ssh):
    ...
    entries_command = "python3.11 /nfs/general/code/EM_data_collection.py "
...
def calibration(ssh):
    ...
    cal_command = "python3.11 /nfs/general/code/EM_data_calibration.py "
    command = "a1"
