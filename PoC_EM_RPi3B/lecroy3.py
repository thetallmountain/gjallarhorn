...
class Lecroy():
    ...
    def connect(self,IPAdrress = "10.205.1.18"):
...
###     self._scope.SetPanel(panel)
    ...
    def getWaveformDescryption(self, channel, use2BytesDataFormat=True):
        ...
        answerString = answerString.replace(':', '')
        voltageGainString = list(filter(None, answerString.split(' ')))[-1]
        ...
        answerString = answerString.replace('"', '')
        answerString = answerString.replace(':', '')
        voltageOffsetString = list(filter(None, answerString.split(' ')))[-1]
        ...
        answerString = answerString.replace('"', '')
        answerString = answerString.replace(':', '')
        timeIntervalString = list(filter(None, answerString.split(' ')))[-1]
        ...
        answerString = answerString.replace('"', '')
        answerString = answerString.replace(':', '')
        timeOffsetString = list(filter(None, answerString.split(' ')))[-1]
    ...
    ###
    def getRawSignal(self, channel, numberOfPoints, firstArray=0, use2BytesDataFormat=False):
    ...
    #
    ###
    def getNativeSignalBytes(self, channel, numberOfPoints, use2BytesDataFormat=False, dataFormat=3):
        startingPoint = 0           # The point to start transfer (0 = first)
        numberOfPointsToJump = 0    # How many points to jump (0 = get all points, 2 = skip every other point) 
        SegmentNumber = 0           # Segment number to get, in case of sequence waveforms.
        # self._scope.SetupWaveformTransfer(startingPoint, numberOfPointsToJump, SegmentNumber)
    ...
    #
    ##
    def setTransfersTimeout(self, seconds):
