...
def unzip_results():
    zip_filename = 'RPi3B_EM_bugs_2024_01_23_14o38.zip'
...
mapeo_bugs = {
    ...
    './Bugs_RPi4/SUT00I.bin': 'SUT00',
    './Bugs_RPi4/SUT00F.bin': 'SUT00'
}
...
def operation(kind):
    ...
    # input_signal[0][:EM_cal_traces] = np.abs(rfft(np.loadtxt(calibration_path + "/" + exec_name,delimiter=',')[:,cut_n_samples:]))
    # input_signal[1][:EM_cal_traces] = np.abs(rfft(np.loadtxt(calibration_path + "/" + noexec_name, delimiter=',')[:,cut_n_samples:]))
    # input_signal[2] = np.abs(rfft(np.loadtxt(results_entries_path + fuzz_name, delimiter=',')[:,cut_n_samples:]))
