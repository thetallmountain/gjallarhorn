...
def clustering_procedure(kind):
    ...
    plt.savefig(save_path + device + "_entries_list_" + date + "_CONFUSION_MATRIX_" + str(get_class_name(db)) + ".png", bbox_inches='tight', dpi=100)
    plt.show()
...
def entries_list_creation(entries):
    ...
    # Repeat each string 20 times and add to the new array
...
start1 = time.time()
# CALIBRATION SIGNALS CAPTURE
# cal_command = "python3.11 EM_data_calibration.py "
# command = "a1"
...
entries_command = "python3.11 EM_data_collection.py "
cal_n_traces = 100
kind = "EM"
